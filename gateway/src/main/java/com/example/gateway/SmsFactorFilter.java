...
@Slf4j
@Component
public class SmsFactorFilter implements GlobalFilter, Ordered {
  ...
  @Autowired
  public SmsFactorFilter(MFAUtil mfaUtil, ApiMfaConfig apiMfaConfig, OktaService oktaService) {
  ...
  public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
    ...
    if (request.getMethod() != null
      ...
      var smsPasscode = headers.getFirst(ApiMfaConfig.X_MFA_SMS);
      ...
      return factorsResponseMono.flatMap(
          factors -> {
            if (factors.getSms() != null && factors.getSms().getStatus().equals("ACTIVE")) {
              if (smsPasscode == null) {
                return mfaUtil.errorResult(
                    mfaUtil.getErrorResponse(factors, "mfa-sms-required"), response);
              }
              return mfaUtil
                  .verifyFactor(smsPasscode, userId, "sms")
                  .flatMap(
